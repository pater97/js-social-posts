...
function stampAll() {
    for (let i = 0; i < posts.length; i++) {
        ...
        stamp.insertAdjacentHTML("beforeend", element)
    }
}
stampAll()
const likeButton = document.getElementsByClassName(`like_button`)
for (let index = 0; index < likeButton.length; index++) {
    const singleButton = likeButton[index]
    singleButton.addEventListener(`click`, function () {
        singleButton.classList.add("click_btn")
    })
}

//arrey post che han ricevuto mi piace, funzione i cui capisco se il bottone ha ricevuto il like, data post id attraverso il this